...
@app.post("/query")
async def query_ai(request: dict):
    ...
    try:
        ...
        if not question or not model:
            raise HTTPException(status_code=400, detail="Question et modèle requis")
        
        if model == "GPT-4o (Azure)":
            result = await query_azure_openai(question)
        elif model == "Claude 3 Haiku":
            result = await query_anthropic(question)
        elif model == "Mixtral 8x7B Instruct":
            result = await query_bedrock(question)
        else:
        ...
        raise HTTPException(status_code=500, detail=f"Erreur inattendue pour {model}: {str(e)}")
